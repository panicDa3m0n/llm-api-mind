...
The first implementation slice should not require embeddings. Start with automatic per-turn lexical retrieval, a relevance guard, runtime context injection, and `memory.context` traces. Dense retrieval and cross-encoder reranking can follow after the automatic pipeline is observable.
...
- SQLite FTS5/BM25 search for the first automatic retrieval stage;
