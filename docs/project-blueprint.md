...
semantic memory, legacy fact audit rows, episodic session recall, one metacognition
...
Scarlet decides autonomously when to inspect schema, search memory, inspect
legacy fact provenance, review possible conflicts, or mutate cognitive state
through traceable operations. A
...
-> reranked selection and exclusions
...
<runtime_context>
{
  ...
  "capabilities": {
    ...
    "memory.conflicts": "review-candidate audit",
...
6. Scarlet-owned duplicate/conflict review over sourceable candidates; the
   backend must not declare semantic conflict from lexical or fact-pattern
   equality alone.
...
- treat backend relation/conflict surfaces as review candidates unless a
  Scarlet-confirmed proposition explicitly establishes semantic authority;
...
V1.64 deliberately has no post-response semantic validator. Source discipline
is Scarlet's model policy and metacognitive responsibility; the runtime
enforces only structural finality, tool lifecycle, persistence, and traces.
...
Historical external ideas to reassess rather than copy directly:

- sourceable propositions confirmed by an explicit semantic authority;
- controlled schemas for storage, not backend inference of meaning;
