...
Unify the Windows Product UI branch with the newer native runtime without
losing either UI behavior or provider recovery, and audit the UI for accidental
Core or cognition changes.

Changes:

- preserved the local `max_tokens` continuation, `tool_use` dispatch,
  `end_turn` finality, five-attempt provider recovery, detached V2 execution,
  and same-turn browser resume;
- retained the Windows real-Core Product UI, paged event replay, responsive
  smoke harness, Windows `tzdata`, Python SDK entrypoint resolution, and CI
  documentation-history fix;
- removed an unpersisted UI-only thinking state and weakened deterministic
  copy that claimed reasoning the event did not prove;
- restored completed provider thinking to Stream V2 as development evidence,
  visible by default and locally hideable without turning it into authored
  speech;
- corrected the fixture-only README boundary and advanced the integrated
  target to V1.55.4.

Verification:

- pre-merge Windows audit worktree: Ruff passed, 305 backend tests passed,
  frontend production build passed;
- isolated SQLite browser smoke passed splash, desktop, mobile, local access
  gate, real Core hydration, session creation, memory, profile/settings, and
  logout without touching laboratory or production databases;
- integrated backend suite: 301 tests passed;
- full Ruff, incremental mypy, documentation integrity, database boundary,
  frontend TypeScript/Vite build, and zero-vulnerability npm audit passed;
- integrated Chrome smoke repeated successfully on desktop `1440x1000` and
  mobile `390x844`; the temporary SQLite database was removed afterward.

Residual Risk:

The local `scarlet/scarlet` gate is not authentication. GitHub push is blocked
on this Mac because the active `DavDit94` credential cannot write to the
`panicDa3m0n/llm-api-mind` remote.
