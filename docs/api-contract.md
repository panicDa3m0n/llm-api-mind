...
This section supersedes historical answer-obligation, automatic atomic-fact,
fact-conflict, and maintenance auto-apply descriptions later in this document.
Those sections remain chronological evidence of earlier contracts.

Current executable rules:

- native Scarlet completes only on provider `end_turn` with non-empty public
  text, completed tool lifecycles, successful persistence, and trace evidence;
- `max_tokens` continuation and interrupted-stream recovery remain transport
  lifecycle behavior;
- GPT bootstrap/action/finalize remain mandatory, but finalize only validates
  a non-empty draft and persists it exactly;
- no runtime component compiles semantic answer obligations or asks a second
  model to accept/reject Scarlet's answer;
- no new `memory_facts` rows are inferred from tags, types, or phrases;
- historical facts remain queryable with `authoritative=false` unless a future
  explicit Scarlet confirmation path records
  `semantic_authority=scarlet, semantic_status=confirmed`;
- legacy facts are excluded from retrieval, reranking, graph recall,
  automatic context, conflict declarations, and maintenance ranking;
- `memory conflicts` returns review candidates, never backend-declared
  semantic conflicts;
- memory maintenance can archive exact normalized duplicates or structurally
  invalid candidates and can attach M2.7 recommendations, but it cannot create
  or otherwise mutate semantic memory; and
- natural-language affect/metacognitive meaning is not inferred from backend
  keyword lists.
...
- ids for sessions, turns, messages, traces, memories, legacy facts, tool calls, and
...
- semantic memory content, semantic `type`, semantic `scope`, reason, and
  expected future use;
- memory/session/legacy-fact audit queries, filters, selected existing target ids,
...
- memory ids, status, provenance, source session/turn/message/trace ids,
  ...
  legacy fact audit rows, retrieval surfaces, graph nodes/edges, embedding vectors, and
...
three-operation Actions transport as the sole external GPT surface. V1.64.0
removes semantic finalize validation while preserving that transport.
...
- returns transport-only `action_policy`, `required_actions`, and
  `recommended_actions`; it does not compile semantic answer obligations.
...
- rejects a whitespace-only draft as a structural input error;
...
- `memory_facts`: legacy structured rows linked to memories, with entity,
  predicate, value JSON, provenance, lifecycle status, and supersession links;
  V1.64 treats them as non-authoritative audit evidence.
...
- The idle job currently runs `sessions.summarize`, missed semantic memory
  review, proposal creation, and one optional batched M2.7 recommendation for
  ambiguous candidates. It never accepts, rejects, merges, supersedes, or
  deprecates semantic memory. Review-ready proposals emit a candidate event
  for the shared Cognitive Workspace.
...
fallback are removed. V1.64 also removes semantic answer obligations entirely.
Grounding remains Scarlet's policy and shell responsibility; the backend
records structural finality from the provider stop reason.
...
Status: compatibility no-op in V1.64
...
Inspect and resynchronize retrieval artifacts for historical fact state without
creating new propositions. Heuristic extraction is retired.
...
```json
{
  "method": "POST",
  "path": "/mind/memory/facts/backfill",
  "intent": "Audit historical fact rows for existing Zero-Luce memories.",
  "body": {
    "memory_id": null,
    "include_inactive": true
  }
...
Return possible relation/divergence candidates for Scarlet's review. The
backend does not declare semantic conflicts. Historical fact rows with the same
entity/predicate and different values may appear as
`legacy_fact_divergence_candidate` with `authoritative=false`; exact duplicate
and retrieval-overlap candidates remain leads, not conclusions.
  ...
  "review_candidates": [
    {
      ...
      "reason": "Historical rows diverge and require source inspection."
    }
  ],
  "trace_ids": ["trace_..."]
}
...
### Memory proposal review through mind_api

Status: implemented in V1.64.0

Purpose:

Let Scarlet inspect source-backed maintenance proposals and make the only
semantic decision in the lifecycle. The backend owns proposal state,
provenance, exact identity, and transactional mutation; M2.7 recommendations
are non-authoritative evidence.
...
Internal dispatch contracts:

```txt
GET  /mind/memory/proposals
GET  /mind/memory/proposals/{proposal_id}
POST /mind/memory/proposals/decide
```

`open` includes `pending` and `pending_review`. `resolved` includes terminal
states only. Proposal reads expose candidate content, source
session/turn/message hooks, related-memory hooks, maintenance recommendation,
and lifecycle state. They do not present the proposal as an active memory.
...
```json
{
  "proposal_id": "prop_...",
  "decision": "accept",
  "reason": "Scarlet inspected the source turn and found this durable.",
  "target_memory_id": null,
  "type": null,
  "scope": null,
  "content": null,
  "expected_future_use": null
}
```

`duplicate` and `supersede` require a target memory selected by Scarlet.
`accept` and `supersede` may correct the proposed type, scope, content, or
future use. Accepted memory preserves the original evidence provenance while
`mind.memory.proposal.decide` records the current Scarlet decision turn.

Terminal states introduced by this contract include
`rejected_by_scarlet`, `resolved_duplicate`, `accepted_create`, and
`accepted_supersede`. A failed write/lifecycle operation leaves the proposal
open rather than claiming completion.
...
for candidate creation, source inspection, and later Scarlet-owned lifecycle
application.
...
processes, and evaluator tooling. Scarlet reaches the same canonical proposal
records through the dedicated `memory proposals` and `memory proposal...`
shell commands documented above.

Query parameters:

```txt
status=open                 default; includes pending and pending_review;
                            resolved expands to terminal states;
                            all/any/* disables it
...
Current V1.64 proposal annotations:

- `needs_semantic_review`: structurally valid candidate awaits Scarlet.
- `noop_duplicate`: an exact normalized active-memory identity exists.
- `review_similar`: a maintenance recommendation only; Scarlet must inspect
  sources and any target memory before deciding.
- `needs_review`: structural validation failed and requires repair.
...
- `pending_review`: M2.7 recommendation recorded without semantic mutation.
- `archived_rejected`: rejected only by deterministic structural policy.
- `archived_noop_duplicate`: archived because exact normalized memory text already exists.
- `applied_create`: historical status; V1.64 maintenance never produces it.
- `accepted_create`: Scarlet accepted and created/reused the proposed memory.
- `accepted_supersede`: Scarlet accepted a replacement and superseded the
  selected active memory.
- `rejected_by_scarlet`: Scarlet rejected the candidate after review.
- `resolved_duplicate`: Scarlet linked the proposal to a selected active
  duplicate without creating memory.
...
structural finality, semantic memory, proposals, compaction, and health checks.
The detailed implementation order is tracked in `docs/memory-roadmap.md`.

### Historical Response-Control And Validation

Status: superseded and removed in V1.64.0. The following V1.41-V1.49 contract
is retained only as historical rationale.
...
### Legacy Atomic Fact Audit Layer

Status: heuristic extraction retired in V1.64.0; stored rows remain audit-only

Purpose:

Preserve structured rows linked to human-readable memory records so provenance
and migration history are not lost. These rows are not active semantic truth.
...
Historical controlled predicates:
...
- `memory facts` can inspect historical rows and reports
  `semantic_status=legacy_heuristic_proposition` and `authoritative=false`;
- `memory facts backfill` is a traceable compatibility no-op and creates zero
  propositions;
- explicit entity/predicate normalization is available only for querying the
  legacy audit table;
- active retrieval, reranking, KG recall, automatic context, relation review,
  and maintenance do not consume legacy fact text; and
- a future confirmed proposition is authoritative only when an explicit
  Scarlet-owned path records `semantic_authority=scarlet` and
