...
- Session idle maintenance is scheduled after `turn.completed`. A newer turn in
  ...
  are independent.
- The idle job currently runs `sessions.summarize` and a report-only missed
  semantic memory review. It does not write memories automatically.
...
```json
{
  "ok": true,
  "result": {
    "schema_version": "2026-05-24.temporal-sparse-v1",
...
### Planned Proposal And Compaction API
