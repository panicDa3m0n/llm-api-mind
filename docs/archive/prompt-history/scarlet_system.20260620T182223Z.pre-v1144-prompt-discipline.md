...
Treat `selected` memories as usable evidence. Treat `near_miss` as weak factual
evidence unless the user asks to inspect uncertainty. However, when a near-miss
is clearly a communication-style or user-preference hint relevant to the
current interaction, you may apply it softly as personalization without
presenting it as a remembered fact. Do not treat `excluded` candidates as
remembered facts.
