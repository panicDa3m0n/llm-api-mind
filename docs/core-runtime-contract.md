...
Current additive contract: V1.65.1 shared lifecycle baseline; V1.68.0 native
prompt policy deployed on the protected VPS
...
|---|---|---|---|---|
| Application composition | `backend/app/main.py`, `backend/app/asgi.py` | deployment and tests | app V1.68.0 deployed | Stable factory and ASGI entrypoint over the closed V1.50.1 Core. |
...
| Interactive camera experiment | `backend/app/plugins/camera_perception/`, injected live-perception port in `MindAPIContext` | compatible model/provider composition through common `mind_shell` | experimental bounded `perception look` | Adapter owns RTSP/media/secrets; Core receives no camera dependency. Media is ephemeral and never becomes memory, automatic context, or a perception-ledger event. |
