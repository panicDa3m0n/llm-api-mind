...
def _add_existing_graph_nodes_and_edges(
    ...
    *,
    ...
    for node in nodes:
        if node.node_type in {"fact", "entity"}:
            # These nodes were generated by the retired lexical fact extractor.
            # Keep them in storage for audit, but exclude them from active recall.
            continue
