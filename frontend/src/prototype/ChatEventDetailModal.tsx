...
export function ChatEventDetailModal({
  ...
  const hasThinkingEvidence = inspection.sourceEvents.some(
    (event) => event.event_type === "llm.thinking.captured"
  );
        ...
        <div className="scarlet-event-detail__body">
          ...
          {hasPrivateEvidence ? (
            <div className="scarlet-event-detail__privacy">
              ...
              <span>
                {hasThinkingEvidence
                  ? "Evidenza di sviluppo visibile. Il thinking ricevuto dal provider è mostrato insieme a ordine e metadati."
                  : "Evidenza privata sbloccata. Presenza, ordine e metadati sono verificabili; i valori sensibili restano oscurati."}
              </span>
            </div>
          ) : null}
