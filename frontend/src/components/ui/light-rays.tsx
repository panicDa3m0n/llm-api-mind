...
const createRays = (count: number, cycle: number): LightRay[] => {
  ...
  return Array.from({ length: count }, (_, index) => {
    const left = 8 + seeded(index, 1) * 84
    const rotate = -28 + seeded(index, 2) * 56
    const width = 160 + seeded(index, 3) * 160
    const swing = 0.8 + seeded(index, 4) * 1.8
    const delay = seeded(index, 5) * cycle
    const duration = cycle * (0.75 + seeded(index, 6) * 0.5)
    const intensity = 0.6 + seeded(index, 7) * 0.5
  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        ...
        />
        {!shouldReduceMotion && rays.map((ray) => (
          <Ray key={ray.id} {...ray} />
        ))}
      </div>
    </div>
