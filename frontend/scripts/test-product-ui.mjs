...
async function runDesktop() {
  ...
  try {
    ...
    await expectVisibleText(page, "Spazio sincronizzato");
    await assertNoHorizontalOverflow(page);
    ...
    if (liveChat) {
      ...
      assert.equal(
        await page.locator('[data-event-type="ui.activity.pending"]').count(),
        0,
        "La UI non deve inventare attività prima di un evento Core."
      );
      await page
    ...
    if (replayFlowSessionId) {
      ...
      await page
        .getByRole("button", { name: "Chiudi dettagli evento" })
        .click();
      const replayThinking = page.locator(
        '[data-event-type="llm.thinking.captured"]'
      );
      await replayThinking.waitFor({ state: "visible" });
      assert(
        (await replayThinking.innerText()).includes("Pensiero di Scarlet"),
        "Il replay deve rendere visibile il thinking di sviluppo."
      );
      await page.screenshot({
        fullPage: false,
    ...
    const privateEvidenceToggle = page.getByRole("button", {
      name: "Evidenze di sviluppo"
    });
    assert.equal(
      await privateEvidenceToggle.getAttribute("aria-pressed"),
      "true",
      "Le evidenze di sviluppo devono essere visibili per default."
    );
    await privateEvidenceToggle.click();
    assert.equal(
      await privateEvidenceToggle.getAttribute("aria-pressed"),
      "false",
      "Lo switch delle evidenze di sviluppo non si è disattivato."
    );
    await page.reload({ waitUntil: "networkidle" });
    await page.getByTestId("profile-screen").waitFor({ state: "visible" });
    assert.equal(
      await page
        .getByRole("button", { name: "Evidenze di sviluppo" })
        .getAttribute("aria-pressed"),
      "false",
      "La preferenza locale delle evidenze di sviluppo non persiste al reload."
    );
    await page
      .getByRole("button", { name: "Evidenze di sviluppo" })
      .click();
    steps.push("development-evidence-setting-persistence");
    ...
    if (replayFlowSessionId) {
      ...
      await eventModal
        .getByText("The interface should not imitate consciousness", {
          exact: false
        })
        .waitFor({ state: "visible" });
      await eventModal
        .getByText("Evidenza di sviluppo visibile", { exact: false })
        .waitFor({ state: "visible" });
      await page
        .getByRole("button", { name: "Chiudi dettagli evento" })
        .click();
      steps.push("thinking-event-visible-inspection");
      await clickDock(page, "Profilo");
      await page.getByTestId("profile-screen").waitFor({ state: "visible" });
    }
